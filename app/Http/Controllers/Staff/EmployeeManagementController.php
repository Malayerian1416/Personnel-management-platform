...
class EmployeeManagementController extends Controller
{
    ...
    public function employee_batch_application(Request $request): array
    {
        ...
        try {
            DB::beginTransaction();
            $response = [];
            $folders = Storage::disk("temporarily")->directories();
            ...
            if ($employees->isNotEmpty()) {
                ...
                $response["data"]["view_data"] = json_encode(["employees" => $employees->pluck("id"), "folder" => $folder, "data" => $request->all()],JSON_UNESCAPED_UNICODE);
            }
            else
                throw ValidationException::withMessages(['employees' => 'پرسنلی جهت انجام عملیات وجود ندارد']);
            DB::commit();
    ...
    public function employee_batch_application_save(Request $request): array
    {
        Gate::authorize('item_batch_application',"EmployeesManagement");
        try {
            DB::beginTransaction();
            ...
            $request->validate([
                ...
                "view_data.required" => "اطلاعات ارسال شده ناقص می باشد",
            ]);
            $form_data = json_decode($request->input("view_data"),true);
            $application_type = $form_data["data"]["application_type"];
            $automation = ApplicationForm::MakeAutomation($application_type);
            $loan_amount = $form_data["data"]["loan_amount"] ? Str::replace(",","",$form_data["data"]["loan_amount"]) : 0;
            $recipient = $form_data["data"]["recipient"] ?: null;
            $borrower = $form_data["data"]["borrower"] ?: null;
            $comment = $form_data["data"]["comment"];
            $form = ApplicationForm::Application($application_type);
            $application = $form->application_class;
            $application = $application == "PAF" ? "PAFBatch" : $application;
            $company_information = CompanyInformation::query()->with("ceo")->first();
            $employees = Employee::query()->with(["contract.organization"])->whereIn("id",$form_data["employees"])->get();
            $employees->map(function ($employee) use ($application,$folder,$company_information,$application_type,$recipient,$loan_amount,$borrower,$automation,$comment) {
                $class = '';
                $data = json_encode([
                    "active_contract" => [
            ...
            $merge = PDFMerger::init();
            foreach (Storage::disk("temporarily")->files($folder) as $file) {
                $merge->addPDF(Storage::disk("temporarily")->path("$file"), [1]);
            }
            $merge->merge();
            $merge->save(Storage::disk("temporarily")->path("{$folder}/{$folder}.pdf"));
            foreach (Storage::disk("temporarily")->files($folder) as $file) {
